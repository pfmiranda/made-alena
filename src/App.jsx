...
const EMPTY_RECIPE = {
  ...
  servings: 4, prepTime: "", bakeTime: "", totalTime: "",
  difficulty: "Medium", calories: 0, totalCalories: 0,
  notes: "", ingredients: [{ amount: "", item: "" }],
  steps: [""], nutritionBreakdown: [], createdAt: "",
};
...
function RecipeCard({ recipe, onClick }) {
  return (
    ...
    >
      ...
      <div style={{ fontWeight: 700, fontSize: 16, color: "#2c1810", marginBottom: 4, lineHeight: 1.3 }}>{recipe.title}</div>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
        <span style={{ fontSize: 11, background: "#fdf3e8", color: "#c0682a", padding: "2px 8px", borderRadius: 20, fontFamily: "sans-serif" }}>{recipe.category}</span>
...
export default function MadeAlena() {
  const [recipes, setRecipes] = useState(() => {
    try {
      const saved = localStorage.getItem("madealena_v1");
  ...
  useEffect(() => {
    try { localStorage.setItem("madealena_v1", JSON.stringify(recipes)); } catch {}
  }, [recipes]);
  ...
  function saveRecipe() {
    ...
    if (form.id) {
      ...
      setRecipes(rs => [...rs, { ...form, id: Date.now(), createdAt: new Date().toISOString() }]);
    }
    setView("grid");
      ...
      <div style={{ minHeight: "100vh", background: "#faf6f0", fontFamily: "Georgia, serif" }}>
        ...
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "20px 16px 60px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: 10, marginBottom: 20 }}>
            {[
              { label: "Prep", val: r.prepTime || "—" },
              { label: "Cook", val: r.bakeTime || "—" },
              { label: "Total", val: r.totalTime || "—" },
          ...
          <div style={{ background: "linear-gradient(135deg, #c0682a, #2c1810)", borderRadius: 16, padding: "16px 20px", color: "#fff", marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 12, opacity: 0.7, fontFamily: "sans-serif" }}>Total batch: {r.totalCalories} kcal</div>
            {r.nutritionBreakdown?.length > 0 && <NutritionBar breakdown={r.nutritionBreakdown} total={r.totalCalories} />}
          </div>
          ...
          {activeTab === "notes" && (
            <div style={{ background: "#fff8f0", borderRadius: 16, padding: "20px", border: "1.5px solid #f0d5b0" }}>
              ...
              <p style={{ margin: 0, color: "#2c1810", lineHeight: 1.7, fontSize: 14 }}>{r.notes || "No notes yet. Edit the recipe to add some!"}</p>
            </div>
          )}
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#faf6f0", fontFamily: "Georgia, serif" }}>
        <div style={{ background: "#2c1810", padding: "16px 20px", display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 10 }}>
          ...
          <button onClick={saveRecipe} style={{ marginLeft: "auto", background: "#c0682a", border: "none", color: "#fff", padding: "6px 16px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>Save</button>
        </div>
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "20px 16px 60px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10 }}>
            {[["prepTime", "Prep"], ["bakeTime", "Cook"], ["totalTime", "Total"], ["servings", "Servings"]].map(([key, lbl]) => (
              <div key={key}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              ...
              <input type="number" style={inputStyle} value={form.totalCalories} onChange={e => setForm(f => ({ ...f, totalCalories: +e.target.value }))} />
            </div>
          </div>
        ...
        <div style={{ maxWidth: 780, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div>
              ...
              <p style={{ color: "#a07850", margin: "2px 0 0", fontSize: 12, fontFamily: "sans-serif" }}>{recipes.length} recipe{recipes.length !== 1 ? "s" : ""} saved</p>
            </div>
